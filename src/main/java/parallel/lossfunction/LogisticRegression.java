...
public class LogisticRegression implements LossFunction {
	
	//need : working_directory, num_map_tasks, num_reduce_tasks, num_variables
	private final Config config;
	
	public LogisticRegression(Config config) {
		this.config = config;
	}
	...
	public static void main(String[] args) throws Exception {
		...
		LogisticRegression lr = new LogisticRegression(config);
		
		Random rand = new Random(1);
		float[] w = new float[config.dataDimensions];
		for(int i=0; i<w.length; i++) { w[i] = rand.nextFloat(); }
